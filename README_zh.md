...
- **心跳巡查** — 定期唤醒 AI Agent 主动检查是否有需要关注的事项；无异常时静默忽略，发现问题时通过 IM 推送告警
...
LobsterAI 内置**心跳巡查**机制，定期自动唤醒 AI Agent，主动检查是否有需要关注的事项——无需用户手动触发。

### 工作原理

1. 每次心跳（默认每 30 分钟）触发一次 Cowork 会话，发送巡查提示词
2. Agent 读取工作目录中的 `HEARTBEAT.md`，按其中的任务列表逐项检查
3. 若无异常，Agent 回复 `HEARTBEAT_OK`——该回复被静默吞掉，不产生任何通知
4. 若发现需要关注的事项，结果将推送到已配置的 IM 渠道

### HEARTBEAT.md

在工作目录中放置 `HEARTBEAT.md`，告诉 Agent 每次巡查时检查哪些内容。示例：

```markdown
# 心跳任务

- 检查过去 30 分钟内是否有新的 GitHub Issue
- 检查收件箱中主题含"紧急"或"ASAP"的邮件
- 如果服务器 CPU 5 分钟负载超过 80%，立刻告警
```

Agent 每次运行都会重新读取该文件，随时修改即可更新巡查内容。
...
|------|------|--------|
| 启用 | 开启或关闭心跳巡查 | 关闭 |
| 巡查间隔 | 15 分钟 / 30 分钟 / 1 小时 / 2 小时 | 30 分钟 |
| 活跃时段 | 仅在指定时段内执行（如 08:00–22:00） | 全天 |
| 通知渠道 | 巡查结果推送到哪些 IM 平台 | 无 |
| 巡查提示词 | 自定义发给 Agent 的提示词 | 内置默认值 |

### 重复抑制

若 Agent 在 24 小时内返回相同内容，重复通知将被静默丢弃，避免告警疲劳。
