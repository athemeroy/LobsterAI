#!/bin/bash
# Get heartbeat configuration and status via LobsterAI internal API.
# Usage: bash "$SKILLS_ROOT/heartbeat-manager/scripts/get-config.sh"
#
...
RESPONSE="$(http_get "${BASE_URL}/api/heartbeat/config")"
CODE=$?
if [ "$CODE" -ne 0 ]; then
  if [ -n "$RESPONSE" ]; then
    echo "$RESPONSE"
