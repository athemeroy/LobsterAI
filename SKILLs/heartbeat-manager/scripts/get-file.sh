#!/bin/bash
# Read HEARTBEAT.md file via LobsterAI internal API.
# Usage: bash "$SKILLS_ROOT/heartbeat-manager/scripts/get-file.sh"
#
# Returns JSON response: { "success": true, "exists": true/false, "content": "..." }
#
...
RESPONSE="$(http_get "${BASE_URL}/api/heartbeat/file")"
CODE=$?
if [ "$CODE" -ne 0 ]; then
  if [ -n "$RESPONSE" ]; then
    echo "$RESPONSE"
