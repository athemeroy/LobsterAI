#!/bin/bash
# Get heartbeat patrol history via LobsterAI internal API.
# Usage: bash "$SKILLS_ROOT/heartbeat-manager/scripts/get-history.sh"
#
# Returns JSON response: { "success": true, "history": [...] }
#
...
RESPONSE="$(http_get "${BASE_URL}/api/heartbeat/history")"
CODE=$?
if [ "$CODE" -ne 0 ]; then
  if [ -n "$RESPONSE" ]; then
    echo "$RESPONSE"
