#!/bin/bash
# Trigger a heartbeat patrol run immediately via LobsterAI internal API.
# Usage: bash "$SKILLS_ROOT/heartbeat-manager/scripts/run-now.sh"
#
# Returns JSON response: { "success": true, "result": { "result": "ok"|"notified"|"error", ... } }
#
...
RESPONSE="$(http_post_json "${BASE_URL}/api/heartbeat/run" "{}")"
CODE=$?
if [ "$CODE" -ne 0 ]; then
  if [ -n "$RESPONSE" ]; then
    echo "$RESPONSE"
