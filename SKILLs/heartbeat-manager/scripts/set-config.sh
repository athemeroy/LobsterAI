#!/bin/bash
# Update heartbeat configuration via LobsterAI internal API.
# Usage: bash "$SKILLS_ROOT/heartbeat-manager/scripts/set-config.sh" '<json_payload>'
#    or: bash "$SKILLS_ROOT/heartbeat-manager/scripts/set-config.sh" @/tmp/config.json
#
# The JSON payload should contain only the fields to update (partial update).
# Returns JSON response: { "success": true, "config": {...}, "status": {...} }
#
...
if [ -z "$1" ]; then
  echo '{"success":false,"error":"No JSON payload provided. Usage: set-config.sh '\''<json>'\'' or set-config.sh @/path/to/file.json"}'
  exit 1
fi
...
RESPONSE="$(http_post_json "${BASE_URL}/api/heartbeat/config" "$PAYLOAD")"
CODE=$?
if [ "$CODE" -ne 0 ]; then
  if [ -n "$RESPONSE" ]; then
    echo "$RESPONSE"
