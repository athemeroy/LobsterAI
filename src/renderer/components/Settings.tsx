...
type TabType = 'general' | 'model' | 'coworkSandbox' | 'coworkMemory' | 'shortcuts' | 'im' | 'email' | 'heartbeat' | 'about';
...
const Settings: React.FC<SettingsProps> = ({ onClose, initialTab, notice }) => {
  ...
  const sidebarTabs: { key: TabType; label: string; icon: React.ReactNode }[] = useMemo(() => [
    ...
    { key: 'heartbeat',     label: i18nService.t('heartbeatTab'),  icon: <SignalIcon className="h-5 w-5" /> },
