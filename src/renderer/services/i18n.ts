...
const translations: Record<LanguageType, Record<string, string>> = {
  zh: {
    ...
    // Heartbeat
    heartbeatTab: '心跳巡查',
    heartbeatEnabled: '启用心跳巡查',
    heartbeatEnabledHint: '定期运行 AI Agent 检查是否有需要关注的事项',
    heartbeatInterval: '巡查间隔',
    heartbeatInterval15m: '15 分钟',
    heartbeatInterval30m: '30 分钟',
    heartbeatInterval1h: '1 小时',
    heartbeatInterval2h: '2 小时',
    heartbeatActiveHours: '活跃时段',
    heartbeatActiveHoursHint: '仅在此时段内执行巡查，留空则全天运行',
    heartbeatActiveHoursStart: '开始时间',
    heartbeatActiveHoursEnd: '结束时间',
    heartbeatNotifyPlatforms: '通知渠道',
    heartbeatNotifyPlatformsHint: '发现异常时通过以下渠道推送通知',
    heartbeatPrompt: '巡查提示词',
    heartbeatPromptHint: '自定义 Agent 巡查时使用的提示词',
    heartbeatStatus: '运行状态',
    heartbeatStatusRunning: '运行中',
    heartbeatStatusStopped: '已停止',
    heartbeatLastRun: '上次运行',
    heartbeatLastResult: '上次结果',
    heartbeatResultOk: '一切正常',
    heartbeatResultNotified: '已发送通知',
    heartbeatResultError: '运行出错',
    heartbeatResultSkipped: '已跳过',
    heartbeatRunNow: '立即运行',
  ...
  en: {
    ...
    heartbeatEnabledHint: 'Periodically run AI Agent to check if anything needs attention',
    heartbeatInterval: 'Check interval',
    heartbeatInterval15m: '15 minutes',
    heartbeatInterval30m: '30 minutes',
    heartbeatInterval1h: '1 hour',
    heartbeatInterval2h: '2 hours',
    heartbeatActiveHours: 'Active hours',
    heartbeatActiveHoursHint: 'Only run during these hours. Leave empty to run all day',
    heartbeatActiveHoursStart: 'Start time',
    heartbeatActiveHoursEnd: 'End time',
    heartbeatNotifyPlatforms: 'Notification channels',
    heartbeatNotifyPlatformsHint: 'Push notifications through these channels when issues are found',
    heartbeatPrompt: 'Check prompt',
    heartbeatPromptHint: 'Customize the prompt used by the Agent during checks',
    heartbeatStatus: 'Status',
    heartbeatStatusRunning: 'Running',
    heartbeatStatusStopped: 'Stopped',
    heartbeatLastRun: 'Last run',
    heartbeatLastResult: 'Last result',
    heartbeatResultOk: 'All clear',
    heartbeatResultNotified: 'Notification sent',
    heartbeatResultError: 'Error',
    heartbeatResultSkipped: 'Skipped',
    heartbeatRunNow: 'Run now',
